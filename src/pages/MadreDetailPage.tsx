...
export function MadreDetailPage() {
  ...
  async function load() {
    ...
    const [{ data: prep, error: prepError }, { data: rubrosData }, { data: config }, { data: listas }] = await Promise.all([
      supabase.from('preparaciones').select('*').eq('id', id).maybeSingle(),
    ...
    setVenue(prep.venue)
    setRubroId(prep.rubro_id ?? '')
    setRendimientoCantidad(prep.rendimiento_cantidad !== null ? String(prep.rendimiento_cantidad) : '')
    setRendimientoUnidad(prep.rendimiento_unidad ?? '')
  ...
  async function handleUpdateCantidad(lineaId: string, value: string) {
    ...
    await supabase.from('preparacion_ingredientes').update({ cantidad_usada: cantidad }).eq('id', lineaId)
  }

  async function handleQuitar(lineaId: string) {
    setLineas((prev) => prev.filter((l) => l.id !== lineaId))
    await supabase.from('preparacion_ingredientes').delete().eq('id', lineaId)
  }

  if (loading) return <p style={{ color: 'var(--rc-text-muted)' }}>Cargando...</p>
  if (notFound) return <p>No se encontró esta Madre.</p>

  return (
    <div>
      <div className="rc-page-header">
        <button className="rc-btn rc-btn-secondary" onClick={() => navigate('/madres')} style={{ marginBottom: '1rem' }}>
          ← Volver a Madres
        </button>
        <h1>{nombre || 'Madre sin nombre'}</h1>
      </div>
      ...
      <div className="rc-card" style={{ marginBottom: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          ...
          <div className="rc-field" style={{ width: 140 }}>
            ...
            <input className="rc-input" value={rendimientoCantidad} onChange={(e) => setRendimientoCantidad(e.target.value)} placeholder="9960" />
          </div>
          <div className="rc-field" style={{ width: 140 }}>
            <label>Unidad rendimiento</label>
            <input className="rc-input" value={rendimientoUnidad} onChange={(e) => setRendimientoUnidad(e.target.value)} placeholder="GRS" />
          </div>
        </div>
